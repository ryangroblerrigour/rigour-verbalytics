...
ENV VARS (set on Render or your platform):
  ...
  - VERBALYTICS_BLOCKLIST_CSV_URL (CSV of global+project-specific blocklist)
...
Respondent's answer: " + response.strip() + "
...
", " ")
    ...
    if contains_blocked_phrase(t, project_id):
        ...
        messages2 = [
            ...
            {"role": "user", "content": (
                "Original question: " + question.strip() + " Respondent's answer: " + response.strip() + " Write ONE follow-up question only."
            )},
        ]
...
@app.get("/health")
def health() -> dict:
    # Light touch refresh so the first call after deploy has blocklist ready
    try:
        _refresh_blocklist(False)
    except Exception:
        pass
...
"
    ...
    return StreamingResponse(event_gen(), media_type="application/jsonl"),media_type="application/jsonl")
