...
def _is_hindi_text(s: Optional[str]) -> bool:
    text = _clean_text(s)
    if not text:
        return False
    devanagari_chars = sum(1 for ch in text if "\u0900" <= ch <= "\u097F")
    letters = sum(1 for ch in text if ch.isalpha())
    return devanagari_chars >= 2 and (devanagari_chars / max(letters, 1)) >= 0.25
...
def _language_bucket(locale: Optional[str], question: Optional[str] = None, response: Optional[str] = None) -> str:
    ...
    if _is_greek_text(question) or _is_greek_text(response):
        return "el"
    if _is_hindi_text(question) or _is_hindi_text(response):
        return "hi"
    return "en"
...
LANG_RULES = {
    ...
    "hi": {
        "empty_values": {"", "na", "n/a", "नहीं पता", "मुझे नहीं पता", "पता नहीं", "कोई नहीं", "कुछ नहीं", "?"},
        "reason_markers": {"क्योंकि", "क्यूंकि", "चूंकि", "इसलिए", "कारण", "वजह", "की वजह से"},
        "emphasis_markers": {"खासकर", "विशेष रूप से", "मुख्य रूप से", "ज्यादा"},
        "first_hand_markers": {
            "मैं इस्तेमाल करता", "मैं इस्तेमाल करती", "मैंने इस्तेमाल किया", "मैं उपयोग करता", "मैं उपयोग करती",
            "मैंने खरीदा", "हमने खरीदा", "मैंने देखा", "मैंने सुना", "मैंने अनुभव किया",
            "हम इस्तेमाल करते", "हम उपयोग करते", "मेरे बच्चे", "मेरा परिवार", "हमारा परिवार"
        },
        "usage_context": {
            "घर पर", "काम पर", "ऑफिस में", "ट्रेन में", "कार में", "रात में",
            "सुबह", "वीकेंड पर", "सप्ताहांत", "स्कूल के बाद", "सफर में",
            "खाना बनाते समय", "सफाई करते समय", "गाड़ी चलाते समय", "सोने से पहले"
        },
        "feature_sensory": {
            "लोगो", "पैक", "पैकेजिंग", "डिजाइन", "डिज़ाइन", "संगीत", "जिंगल",
            "आवाज", "वॉइसओवर", "सीन", "दृश्य", "अभिनेता", "किरदार", "चरित्र",
            "स्वाद", "गंध", "खुशबू", "बनावट", "रंग", "कीमत", "ऑफर", "छूट",
            "टिकाऊ", "बैटरी", "गति", "स्पीड", "निर्देश", "इंटरफेस", "ऐप",
            "ध्वनि", "आवाज़", "वॉल्यूम", "गुणवत्ता", "क्वालिटी", "रिजॉल्यूशन",
            "कैमरा", "लेबल", "स्लोगन", "टैगलाइन"
        },
        "example_markers": {"उदाहरण के लिए", "जैसे", "मसलन", "खासकर", "विशेष रूप से"},
        "opinion_q": {"सोचते", "राय", "महसूस", "लगता", "पसंद", "नापसंद", "छाप", "इम्प्रेशन", "रेट", "मूल्यांकन"},
        "subjective_resp": {
            "अच्छा", "अच्छी", "बुरा", "बुरी", "बहुत अच्छा", "पसंद", "नापसंद",
            "बेहतरीन", "खराब", "मजेदार", "उबाऊ", "यादगार", "भ्रमित", "स्पष्ट",
            "उपयोगी", "चिड़चिड़ा", "परेशान", "आनंद", "अद्भुत", "कमजोर", "मजबूत"
        },
        "refers_to_item": {"विज्ञापन", "ऐड", "एड", "कमर्शियल", "स्पॉट", "यह", "ये", "इस"},
        "fillers": {"मतलब", "असल में", "थोड़ा", "थोड़ी", "कुछ हद तक", "आप जानते हैं", "चीजें", "वगैरह"},
        "templ": [
            "एक ai के रूप में", "एक एआई के रूप में", "भाषा मॉडल के रूप में",
            "कुल मिलाकर", "इसके अलावा", "सारांश में", "निष्कर्षतः", "महत्वपूर्ण रूप से"
        ],
        "invalid_short": {"ठीक", "हाँ", "हां", "नहीं", "शायद", "पता नहीं", "मुझे नहीं पता", "कुछ नहीं", "कोई नहीं"},
        "fallback_short_followup": "क्या आप इसके बारे में थोड़ा और विस्तार से बता सकते हैं?",
        "fallback_followup": "आप ऐसा क्यों कहते हैं?",
        "repair_numeric": "क्या आप समझा सकते हैं कि '{value}' से आपका क्या मतलब है, इस सवाल के संदर्भ में: {question}",
        "repair_general": "क्या आप इस सवाल के संदर्भ में थोड़ा और विस्तार से बता सकते हैं: {question}",
        "deepdive_questions": {
            "low_score": {
                "ask_why_that_wording": "उस शब्दावली या वाक्यांश में ऐसा क्या है जो आपके लिए काम नहीं करता?",
                "ask_which_words": "किन शब्दों या वाक्यांशों ने आपको ऐसा महसूस कराया?",
                "ask_specific_words": "टोन या शब्दावली में खास तौर पर क्या आपके लिए काम नहीं किया?",
                "ask_which_visuals": "विजुअल्स या डिजाइन में खास तौर पर किस चीज ने आपको ऐसा महसूस कराया?",
                "ask_specific_visuals": "विजुअल्स या डिजाइन में खास तौर पर क्या आपके लिए काम नहीं किया?",
                "ask_meaning": "संदेश या अर्थ में क्या आपको सही नहीं लगा?",
                "ask_specifics": "खास तौर पर क्या आपके लिए काम नहीं किया?",
            },
            "mid_score": {
                "explore_both_sides_words": "शब्दावली के कौन से हिस्से आपके लिए काम किए और कौन से नहीं?",
                "explore_both_sides_visuals": "विजुअल्स या डिजाइन में क्या आपके लिए काम किया और क्या नहीं?",
                "explore_both_sides": "आपके लिए क्या काम किया और क्या नहीं?",
            },
            "high_score": {
                "ask_why_that_wording": "उस शब्दावली या वाक्यांश में ऐसा क्या था जो आपको अच्छा लगा?",
                "ask_specific_words": "शब्दावली या टोन में खास तौर पर क्या आपको अच्छा लगा?",
                "ask_specific_visuals": "विजुअल्स या डिजाइन में खास तौर पर क्या आपको अच्छा लगा?",
                "ask_meaning": "संदेश या अर्थ में क्या आपसे जुड़ा?",
                "ask_specifics": "खास तौर पर किस चीज ने इसे आपके लिए प्रभावी बनाया?",
            },
            "neutral": {
                "ask_which_words": "किन शब्दों या वाक्यांशों ने आपको ऐसा महसूस कराया?",
                "ask_specific_words": "शब्दावली या टोन में खास तौर पर क्या आपके लिए अलग लगा?",
                "ask_specific_visuals": "विजुअल्स या डिजाइन में खास तौर पर क्या आपके लिए अलग लगा?",
                "ask_meaning": "संदेश या अर्थ में क्या आपके लिए अलग लगा?",
                "clarify": "आपका इससे ठीक-ठीक क्या मतलब है?",
            }
        },
        "deepdive_eval": {
            "criticism_words": [
                "सामान्य", "फीका", "अस्पष्ट", "भ्रमित", "ज्यादा इस्तेमाल",
                "कमजोर", "उबाऊ", "बहुत ज्यादा", "बहुत कम", "ढीला", "बेअसर"
            ],
            "mentions_language": [
                "टोन", "लहजा", "शब्द", "वाक्यांश", "फ्रेज", "शब्दावली",
                "भाषा", "संदेश", "लाइन", "टैगलाइन", "कॉपी"
            ],
            "mentions_visuals": [
                "रंग", "विजुअल", "विजुअल्स", "डिजाइन", "डिज़ाइन", "लेआउट",
                "लुक", "फॉन्ट", "छवि", "तस्वीर", "इमेज", "चित्र"
            ],
            "mentions_meaning": [
                "अर्थ", "मतलब", "दिखाता", "लगता", "संकेत", "इशारा",
                "महसूस", "लगा", "समझ आता", "समझ नहीं आता"
            ],
            "mentions_relevance": [
                "मेरे लिए", "मुझसे", "व्यक्तिगत रूप से", "प्रासंगिक",
                "जुड़ता", "नहीं जुड़ता", "कनेक्ट", "संबंध"
            ],
        },
    },
}
...
# FastAPI app
# ---------------------------------------------------------------------
app = FastAPI(title="Verbalytics API", version="2.4.1")
